...
Entity_Count_Active_Subsc = session.query(
    PCC_Subscription.Entity_ID,
    func.count(PCC_Subscription.subscription_status).label("Count"),
).filter(PCC_Subscription.subscription_status == "Active").group_by(PCC_Subscription.Entity_ID, PCC_Subscription.subscription_status)

Corp_Count_Active_Subsc = session.query(
...
Cust_Count_Active_Subsc = session.query(
...
# Insert into DB
for i in Entity_Count_Active_Subsc:
    session.add(
        PCC_ENTITYAGG_DATA(
            Entity_ID=i.Entity_ID, CNT_ACTIVE_SUBSC_IC=i.Count
        )
    )

for i in Corp_Count_Active_Subsc:
    session.add(
        PCC_CORPAGG_DATA(Corp_ID=i.Corp_ID, CNT_ACTIVE_SUBSC_IC=i.Count)
    )

for i in Cust_Count_Active_Subsc:
    session.add(
        PCC_CUSTAGG_DATA(Customer_ID=i.Customer_ID, CNT_ACTIVE_SUBSC_IC=i.Count)
    )
