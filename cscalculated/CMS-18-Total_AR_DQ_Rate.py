...
Sum_Balance_of_Accounts = session.query(func.sum(PCC_Account.outstanding_balance))[0][0]
...
for i in Group_Balance_DQ_Accounts:
    session.add(
        PCC_COMPGAGG_DATA(Group_ID=i.Group_ID, TOT_AR_DQRATE=Group_AR_DQRate)
    )

for i in Entity_Balance_DQ_Accounts:
    session.add(
        PCC_ENTITYAGG_DATA(
            Entity_ID=i.Entity_ID, TOT_AR_DQRATE=Entity_AR_DQRate
        )
    )

for i in Corp_Balance_DQ_Accounts:
    session.add(
        PCC_CORPAGG_DATA(Customer_ID=i.Customer_ID, TOT_AR_DQRATE=Corp_AR_DQRate)
    )

for i in Cust_Balance_DQ_Accounts:
    session.add(PCC_CUSTAGG_DATA(GBU_ID=i.GBU_ID, TOT_AR_DQRATE=Cust_AR_DQRate))
