...
GBU_Count_Terminated_Subsc = session.query(
    PCC_Subscription.GBU_ID,
    func.count(PCC_Subscription.subscription_status).label("Count"),
).filter(PCC_Subscription.subscription_status == "Terminated").group_by(PCC_Subscription.GBU_ID, PCC_Subscription.subscription_status)

Group_Count_Terminated_Subsc = session.query(
    PCC_Subscription.Group_ID,
    func.count(PCC_Subscription.subscription_status).label("Count"),
).filter(PCC_Subscription.subscription_status == "Terminated").group_by(PCC_Subscription.Group_ID, PCC_Subscription.subscription_status)
...
# Insert into DB
for i in GBU_Count_Terminated_Subsc:
    session.add(
        PCC_GBUAGG_DATA(GBU_ID=i.GBU_ID, CNT_TERMSUBSC_CC=i.Count)
    )

for i in Group_Count_Terminated_Subsc:
    session.add(
        PCC_COMPGAGG_DATA(Group_ID=i.Group_ID, CNT_TERMSUBSC_CC=i.Count)
    )

for i in Entity_Count_Terminated_Subsc:
    session.add(
        PCC_ENTITYAGG_DATA(
            Entity_ID=i.Entity_ID, CNT_TERMSUBSC_CC=i.Count
        )
    )

for i in Corp_Count_Terminated_Subsc:
    session.add(
        PCC_CORPAGG_DATA(Corp_ID=i.Corp_ID, CNT_TERMSUBSC_CC=i.Count)
    )

for i in Cust_Count_Terminated_Subsc:
    session.add(
        PCC_CUSTAGG_DATA(Customer_ID=i.Customer_ID, CNT_TERMSUBSC_CC=i.Count)
    )
