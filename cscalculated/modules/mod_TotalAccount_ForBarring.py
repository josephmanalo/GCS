...
def TotalAccount_ForBarring(session):
    """
    ...
    gbu_data = (
        ...
        .filter(PCC_Account.collection_status == "Barring Requested")
    ...
    for i in gbu_data:
        session.add(PCC_GBUAGG_DATA(GBU_ID=i.GBU_ID, TOT_ACC_FORBAR=i.Count))

    for i in grp_data:
        session.add(PCC_COMPGAGG_DATA(Group_ID=i.Group_ID, TOT_ACC_FORBAR=i.Count))

    for i in leg_data:
        session.add(PCC_ENTITYAGG_DATA(Entity_ID=i.Entity_ID, TOT_ACC_FORBAR=i.Count))

    for i in crp_data:
        session.add(PCC_CORPAGG_DATA(Corp_ID=i.Corp_ID, TOT_ACC_FORBAR=i.Count))

    for i in cst_data:
        session.add(PCC_CUSTAGG_DATA(Customer_ID=i.Customer_ID, TOT_ACC_FORBAR=i.Count))
