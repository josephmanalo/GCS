...
def Count_TerminatedSubscription_IndivCustomer(session):
    """
    ...
    try:
        ...
        for i in leg_data:
            session.add(PCC_ENTITYAGG_DATA(Entity_ID=i.Entity_ID, CNT_TERMSUBSC_IC=i.Count))
    ...
    try:
        ...
        for i in crp_data:
            session.add(PCC_CORPAGG_DATA(Corp_ID=i.Corp_ID, CNT_TERMSUBSC_IC=i.Count))
    ...
    try:
        ...
        for i in cst_data:
            session.add(PCC_CUSTAGG_DATA(Customer_ID=i.Customer_ID, CNT_TERMSUBSC_IC=i.Count))
