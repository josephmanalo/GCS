...
def CountActiveAccounts(session):
    ...
    try:
        gbu_data = (
            session.query(
                PCC_Account.GBU_ID, func.count(PCC_Account.account_status).label("Count")
            )
    ...
    try:
        grp_data = (
            session.query(
                PCC_Account.Group_ID, func.count(PCC_Account.account_status).label("Count")
            )
            .filter(PCC_Account.account_status == "Active")
    ...
    try:
        leg_data = (
            session.query(
                PCC_Account.Entity_ID, func.count(PCC_Account.account_status).label("Count")
            )
            .filter(PCC_Account.account_status == "Active")
    ...
    try:
        crp_data = (
            session.query(
                PCC_Account.Corp_ID, func.count(PCC_Account.account_status).label("Count")
            )
            .filter(PCC_Account.account_status == "Active")
